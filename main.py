...
def parse_table_data(table):
    ...
    for row in rows[1:]:
        ...
        if cols:
            ...
            local_price_increase = extract_local_city_price(cols[1])
            ...
            if product_name:
                ...
                for index, col in enumerate(cols[2:], start=2):
                    price_increase = extract_other_city_price(col)
                    if price_increase != 0:
...
# 提取本市所卖商品的增幅
def extract_local_city_price(col) -> int:
...
def main():
    ...
    for deal in sorted_best_deals[:3]:
        print(
            f"商品名: {deal['商品名']}, 出发: {deal['本地城市']}, 到达: {deal['目标城市']}, 增幅结果: {deal['增幅结果']}%")
